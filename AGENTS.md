...
6. `internal/runner` executes each check via `sh -c` (local runner) or in an ephemeral docker container; `internal/scoring` and `internal/gradebook` turn results into scores. A check that declares `parser:` also has its run-phase report read by `internal/testreport`, which turns `go test -json`/JUnit XML/TAP into per-case rows and makes the check worth the passed proportion of its weight (SPEC §4.3). A gate is still decided by the exit code alone. A check is one phase or two: when any check of the task declares `build:`, every build phase runs first, the hidden-test files are then removed from the workspace, and only then do the run phases execute (SPEC §6.1). A build phase's log is teacher-only - it is the one that compiles against the hidden tests - and lives in `logs/<id>/build/`.
...
- `internal/ratelimit`: one failure-only limiter instance shared between the web login form, git HTTP basic auth and the OIDC callback, keyed by (client IP, login).
- `internal/testreport`: the three report parsers, stdlib-only and importing nothing else in the tree (`config` validates the `parser:` name against it, `runner` calls it). Everything it reads is written by a run of student code, so it bounds what it stores and never turns its own failure into the check's: an unreadable report leaves the exit code in charge.
- `internal/oidc`: the optional OpenID Connect relying party (discovery, PKCE, ID token verification against the issuer's JWKS). Stdlib only, no store, no handlers - `web` gets a `*oidc.Provider` and stays free of protocol detail. Credentials come only from the environment (`ANYGRADE_OIDC_*`), never from the course repo, never from argv. Nil provider = the feature does not exist: no button, no routes. `internal/oidc/oidctest` is a fake issuer shared by the protocol, handler and e2e tests.
