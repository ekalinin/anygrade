...
- An external identity provider as the account model. A generic OpenID Connect login exists and is optional (§8), but it is only a second way to obtain a session: it never creates an account, and it cannot replace the personal token, which is also the git password. Per-vendor integrations - including providers that are OAuth but not OIDC, such as GitHub - stay out of scope.
...
The tasks root directory is configurable, and must stay inside the repo; a task is any directory under it containing `task.yaml`.
...
- `anygrade validate` verifies all metadata (unknown fields, missing files in `solution_files`, symlinked entries in `solution_files`/`workspace.include`, a `tasks_dir` that is absolute or escapes the repo, deadline ordering, duplicate task ids, negative check weights, an unknown `parser:` or an escaping `parser_file:`, non-positive size limits, credentials embedded in a hidden-tests url, a hidden-tests url or ref that git would read as an option, an enrolment window that closes before it opens, a negative `registration.max_accounts`) and is also run at server startup; startup fails on invalid metadata. Warnings are reported by `validate` alone; they never fail a startup or a teacher push.
...
- OpenID Connect (optional, off unless configured): a login at an external identity provider, for a university that already runs one and does not want one more password per student. Generic OIDC only - one correct relying party covers a university IdP, Google and every Keycloak/Okta/Entra deployment, where per-vendor integrations would be a new code path each. Discovery via the issuer's `.well-known/openid-configuration`; authorization code flow with PKCE; the ID token's signature is verified against the issuer's JWKS and its `iss`, `aud`/`azp`, `exp`, `iat` and `nonce` are all checked. Configuration is environment-only (§11), never `course.yaml`.
  - **It replaces the login form, not the token.** The same token is the git-over-HTTP basic-auth password (§7), and git has nowhere to run a browser redirect - so a provider login is a second way to obtain a session, not a second kind of account. An account that arrived this way still needs a token to push, and gets it from the settings page: the same "regenerate token" action, which is a first issue when the account has none and a rotation when it has one, with the same shown-once display activation uses. The page says what a rotation costs, because it invalidates the old token immediately and a git remote that stored it stops working until the saved password is updated.
  - **Bind only, never create.** A successful login binds to an account that already exists; it does not make one. Otherwise the provider becomes the enrolment gate and `registration.mode: invite` means nothing - a course-policy decision the operator did not make by configuring an IdP. A verified identity with no account is told to ask its teacher for an invite, and that is all it gets.
  - **The binding is `iss` + `sub`**, stored on the `users` row (§12) - not the email and not the login, either of which the provider may rename. An identity nobody is bound to is matched once against an account login by a configurable claim (`preferred_username` by default; when it is `email`, the issuer must also mark it verified), and the subject is recorded then. From then on the subject alone resolves the account, so a rename on the provider's side costs nothing.
  - An account that already carries one subject never accepts another: a second claimant is refused and audited, because whoever the provider hands that login to next would otherwise be one sign-in away from somebody else's submissions. A teacher clears a binding with `anygrade user unbind-oidc` (§11), and the next successful login re-binds.
  - A disabled account gets no session here either. The state filter is the same one the token, session and SSH-key lookups carry.
  - The session a provider login opens is bound to no token (§12): it was not obtained from one, and an invited-but-never-activated account has none. Rotating a token therefore does not end it - deactivating the account does, which is the revocation lever either way.
  - With no configuration nothing changes: the login page shows no button, and the `/oidc/` routes do not exist.
...
- `invite`: the teacher creates the accounts and the students activate them. Two CLI commands cover the two ways an account can start (§11):
  - `anygrade user invite` creates the account and prints a one-time link, for one login or for a whole roster via `--csv`. The student opens the link, is issued a token, and gets their repo URL (SSH keys come afterwards, from settings). This is the normal path. The link is consumed before the account is activated, so a failed activation never leaves a reusable link behind - the teacher issues a new one. An account that already holds a token, or is bound to an identity provider - which has no token yet and takes its first from the settings page - cannot be invited again: activating it a second time issues a token over the credential its owner is using and logs whoever opens the link in as them, so a single `--login` is refused and a roster reports the login, skips it and exits non-zero. A link still outstanding when the account gains either credential stops working for the same reason, while an account that was never activated is re-invited freely - the new link replaces the old one;
...
- A submission's checks carry the per-test-case detail of a check with a `parser:` (§4.3): `cases` in report order (name, status, duration, message), the `passed_cases`/`scored_cases` tally its proportional weight was computed from, and `parse_failed` for a report that could not be read - which is the only thing that tells a fallback to the exit code apart from a check that never had cases. `cases` is present and empty for a check without a parser, so its type does not depend on the course metadata. The earned weight itself is not a field: computing it needs the check's gate flag, which `check_results` does not store (§12), so the counts are published and the division is the client's. The cases are inline rather than an endpoint of their own because they are already loaded with the check results the page renders, and §4.3 bounds them at rest.
...
anygrade user    add|list|invite|deactivate|reactivate
                 reset-token|add-key|unbind-oidc ...
                              ...
                              # deactivate/reactivate: switch every credential
                              #      path of an account off and back on,
                              #      deleting nothing; `remove` is a deprecated
...
- `user deactivate --login LOGIN` and `user reactivate --login LOGIN` are the CLI half of the state switch the teacher UI offers (§8), and write the same `user.state` audit event it writes. The actor is empty: the CLI has no session, so the action is attributed to nobody rather than to the account it targets, and the audit page renders it as a system row with an unknown role (§12). `user remove` remains as a deprecated alias of `deactivate` - it never removed anything - and prints a deprecation note on stderr.
...
- `users` (id, login, display_name, role, state, created_at, oidc_iss, oidc_sub) - the last two are the identity provider binding (§8), NULL for every account that has never signed in through one. `UNIQUE(oidc_iss, oidc_sub)` keeps one provider identity reachable by at most one account; SQLite treats NULLs in a unique index as distinct, so unbound accounts coexist freely
...
- `invites` (token_hash, user_id, expires_at, used_at) - one live activation link per account: `user_id` is unique and a re-invite is an upsert, so a roster re-run replaces the account's outstanding link instead of leaving a second one that activates it again. A database that already carried several keeps the newest, the link the teacher last handed out
- `sessions` (id_hash, user_id, token_hash, created_at, expires_at) - web sessions; the cookie value is stored hashed, like tokens and invites, so the table is not a set of usable cookies. A hash cannot be derived from the values already stored, so the migration that introduced it empties the table: an upgrade signs everybody out once. `token_hash` is NULL for a session an identity provider opened (§8) - it was not obtained from a token, and the account may have none - and a NULL binding is what makes such a session survive a token rotation; the account's state still ends it
...
- Teacher cancel is final: it stops the run whichever moment it arrives in - including while the submission only waits on a retry backoff, where the queue view offers the same cancel - and a retry already in flight never re-arms the canceled submission.
- Check timeout: the container/process is killed, the check fails with a `timed out after 5m` note; remaining checks still run unless the timed-out check was a gate. The timeout is per phase (§4.3), so a two-phase check that hangs in its build phase is killed there and never reaches its run phase, and the docker container's own lifetime is sized by the number of phases rather than the number of checks. On the local runner, a descendant that escapes the process group (for example by calling `setsid`) is not reached by the group kill and can keep the check's output pipe open; the runner gives up waiting for it after a short grace period rather than blocking the worker forever. A check that exits cleanly on its own is still graded by its exit code even when that grace period is the reason its pipe closed late.
...
- A check whose `parser:` found no report - the wrong format, no such file, output that parses to no case at all, a report over the bounds of §4.3, or a report cut short by `runner.log_max`: the check keeps the verdict of its exit code, the row is marked, and the page says the per-test-case report could not be read. A course that loses its parser this way scores exactly as it did before it had one.
...
- The local runner enforces only the wall-clock timeout (process-group kill); memory/cpu limits are docker-only, and `validate` warns when a local-runner task sets them. It has no privilege drop either: checks run as whatever user anygrade runs as. It passes the host environment through to the check unfiltered except for the server's own `ANYGRADE_*` variables, which are dropped before the artifacts path is exported: this keeps the server's credentials out of the check log and the live stream, not a boundary, since a check running as the server's own uid could read them from `/proc` anyway.
...
- The OpenID Connect callback (§8) is on that per-IP budget too, and deliberately not because the authorization code is guessable - it is not, and redeeming one is the provider's own problem. It is because a failing callback is the only unauthenticated route on the site that makes the server open an outbound connection to a third party, so the budget is what bounds how many of those an anonymous peer can extract from it. A completed login clears the key, so a student who fumbles their password at the provider is not throttled afterwards.
- Everything on the provider flow is unauthenticated until the ID token verifies. The state, the nonce and the PKCE verifier live in one short-lived `HttpOnly` cookie scoped to `/oidc`, which is what binds the flow to the browser that started it: the state travels through the provider and the address bar, so on its own it would let anybody holding a code log somebody else's browser in. Reading the cookie clears it, which is what makes the state and the nonce single-use. The post-login target is validated as a same-site absolute path, so the callback cannot be turned into an open redirect. A refusal never tells the browser which check failed - wrong issuer, bad signature, replayed nonce, disabled account and no account at all are two neutral messages between them; the detail goes to the server log, the way hidden-tests errors are scrubbed (§11).
- The ID token is verified against the issuer's published keys with an algorithm allowlist (RSA and ECDSA signatures only). `alg: none` and the HMAC family have no entry in it, so an unsigned token and one signed with a symmetric key derived from the client secret never reach a code path that could accept them.
