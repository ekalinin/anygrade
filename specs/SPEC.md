...
- Errors are `{"error": {"code": ..., "message": ...}}` with a stable code (`unauthorized`, `not_found`, `method_not_allowed`, `rate_limited`, `internal`). The message is for a human reading a log and is deliberately not localized (§10.1): a script parses the code. Every refusal under `/api/v1/` is that envelope, including an unknown path (`404`) and a verb other than `GET` on a known one (`405`, with `Allow: GET`), so a client that always decodes JSON never meets a plain-text one.
